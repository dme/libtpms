...
/*
 * The format of the blob generated by this function is defined in
 * tpm_blobs.h and intended to be stable from version to version.
 */
UINT16
PCRValues_Marshal(BYTE **buffer, INT32 *size)
{
    UINT32 pcr, pcr_count, version;
    UINT16 written;
    UINT32 alg_count[IMPLEMENTATION_PCR] = { 0 };
	...
	alg_count[pcr] = 0;
	for (alg = 0; alg < gp.pcrAllocated.count; alg++)
	    if (GetPcrPointer(gp.pcrAllocated.pcrSelections[alg].hash, pcr) != NULL)
		alg_count[pcr]++;
    ...
    written = 0;
    version = TPMLIB_BLOB_PCR_VALUES_VERSION_1;
    written += UINT32_Marshal(&version, buffer, size);
    written += UINT32_Marshal(&pcr_count, buffer, size);
	...
	written += UINT32_Marshal(&pcr, buffer, size);
	written += UINT32_Marshal(&alg_count[pcr], buffer, size);

	for (alg = 0; alg < gp.pcrAllocated.count; alg++) {
	    BYTE *pcrData;

	    pcrData = GetPcrPointer(gp.pcrAllocated.pcrSelections[alg].hash, pcr);
	    if (pcrData != NULL) {
		const char *alg_name =
		    GetDigestNameByHashAlg(gp.pcrAllocated.pcrSelections[alg].hash);
		UINT32 pcrSize =
		    CryptHashGetDigestSize(gp.pcrAllocated.pcrSelections[alg].hash);

		if (alg_name == NULL)
		    alg_name = "unknown";

		written += String_Marshal(alg_name, buffer, size);
		written += UINT32_Marshal(&pcrSize, buffer, size);
		written += Array_Marshal(pcrData, pcrSize, buffer, size);
	    }
	}
    }
 ...
 */
UINT16
PCREventLog_Marshal(BYTE **buffer, INT32 *size)
{
    UINT32 version;
    UINT16 written;
    TPMI_PCR_EVENT *ple;
    ...
    written = 0;
    version = TPMLIB_BLOB_PCR_EVENT_LOG_VERSION_1;
    written += UINT32_Marshal(&version, buffer, size);
    written += UINT32_Marshal(&pcr_log_count, buffer, size);

    ple = pcr_log_head;
    while (ple != NULL) {
	const char *alg_name = GetDigestNameByHashAlg(ple->alg);
	UINT32 pcrSize = CryptHashGetDigestSize(ple->alg);

	if (alg_name == NULL)
	    alg_name = "unknown";

	written += UINT32_Marshal(&ple->pcrNumber, buffer, size);
	written += TPM_ALG_ID_Marshal(&ple->alg, buffer, size);
	written += String_Marshal(alg_name, buffer, size);
	written += UINT32_Marshal(&pcrSize, buffer, size);
	written += Array_Marshal(ple->data, pcrSize, buffer, size);
	written += UINT32_Marshal(&ple->sequence_number, buffer, size);
